...
const isDuration = function isDuration(d) {
...
const wrapper = function wrapper(input, instance, unit) {
  return new Duration(input, unit, instance.$l)
}

const prettyUnit = function prettyUnit(unit) {
  return `${$u.p(unit)}s`
}

const isNegative = function isNegative(number) {
  return number < 0
}
...
const absolute = function absolute(number) {
  return Math.abs(number)
}

const getNumberUnitFormat = function getNumberUnitFormat(number, unit) {
  if (!number) {
    return {
      negative: false,
      format: '',
